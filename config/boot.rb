...
Padrino.before_load do
  ...
  Padrino.load_paths.concat ["/lib"].map{|v| Padrino.root + v }
  ActiveSupport::Dependencies.autoload_paths += ["/lib"].map{|v| Padrino.root + v }
  require 'hc/config'
end
...
Padrino.after_load do
  Dir.glob(Padrino.root + "/config/initializers/*").each do |initlib|
    load initlib
    STDERR.puts("load done: #{initlib}")
  end
end
